...
const Card = ({ data, reference }) => {
  return (
    <motion.div
      drag
      dragConstraints={reference}  whileDrag={{scale:1.18}}
      className="w-[16vw] h-[45vh] rounded-[2rem] text-white px-5 py-10 cursor-pointer bg-black relative overflow-hidden"
    >
      ...
      <div className="footer  absolute bottom-0 left-0 w-full h-15  ">
        <div className="flex items-center justify-between p-5">
          ...
          <span className="w-7 h-7 bg-zinc-500 rounded-full flex items-center justify-center cursor-pointer">
            {/* <LuDownload /> */}
            {data.close ? <IoCloseSharp /> : <LuDownload />: <IoCloudUploadOutline />}
          </span>
        </div>{" "}
